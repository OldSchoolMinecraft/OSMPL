...
class UtilModule(pl: OsmPlugin) : OsmModule("Util", 1.0F, pl) {
    ...
    init {
        commands.addAll(setOf(
                DISCORD, WARNINGS_COMMAND, HAT_COMMAND, LIST_COMMAND, OSM_COMMAND, SLEEPING_COMMAND, SLOWMODE_COMMAND, STAFF_COMMAND, DONATE_COMMAND, RAFFLE_COMMAND, BAN_COMMAND, TEMP_BAN_COMMAND, SEEN_COMMAND, UN_BAN_COMMAND, PLAYER_MANAGER, LANDMARKS_COMMAND, MUTE_COMMAND, TEMP_MUTE_COMMAND, UN_MUTE_COMMAND, PLAY_TIME_TOP, BAL_TOP, VIEW_PUNISHMENTS, WARN_COMMAND, RAINBOW, DISABLE_QUESTS, PARTICLE_EFFECT, WILD_COMMAND
        ))
    }
    ...
    override fun onEnable() {
        ...
        sequenceOf(ENTITY_DEATH, STAFF_DISABLE, PLAYER_DATA_MANAGER, PLAYER_CHAT, SLOW_MODE_AUTO_TOGGLE, ON_VIEW_LOCKETTE, BLOCK_PLACE, MOVE_HANDLER)
