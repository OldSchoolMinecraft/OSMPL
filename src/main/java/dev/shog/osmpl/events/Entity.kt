 ...
 */
internal val ENTITY_DEATH = { osm: OsmPl ->
    osm.server.pluginManager.registerEvent(Event.Type.ENTITY_DEATH, object : EntityListener() {
        override fun onEntityDeath(event: EntityDeathEvent?) {
            if (event != null && event.entity is Player) {
                ...
                val message = osm.configuration.getString("playerDeath")
                        .replace("{0}", player.displayName)
                        .replace("{1}", reason)

                osm.server.broadcastMessage(message)
            }
        }
