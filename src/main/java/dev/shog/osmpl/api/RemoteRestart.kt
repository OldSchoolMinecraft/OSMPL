 ...
 */
class RemoteRestart(private val pl: OsmPl, private val container: MessageContainer) : NanoHTTPD(8010) {
    init {
                ...
                session.uri.equals("/restart", true) && session.method == Method.POST -> {
                    ...
                    if (rs.next()) {
                        ...
                        if (rs.getInt("admin") == 1 && sqlPass.equals(password, true)) {
                             pl.server.broadcastMessage(container.getMessage("restart"))
    ...
    private fun getPostData(ses: IHTTPSession): ConcurrentHashMap<String, String> {
